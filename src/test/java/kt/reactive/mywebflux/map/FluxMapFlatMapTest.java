package kt.reactive.mywebflux.map;
...
public class FluxMapFlatMapTest {
    ...
    public void stringMapFlatMap() {
        ...
        var map = Flux.fromArray(new String[]{"Tom", "Melissa", "Steven", "Megan"})
                .map(name -> Mono.just(name.concat(" modified"))); // map 내부 처리는 String으로 이루어지나 다시 Mono로 싸진다.
        map.subscribe(System.out::println);

        var flatMap = Flux.fromArray(new String[]{"Tom", "Melissa", "Steven", "Megan"})
                .flatMap(name -> Mono.just(name.concat(" modified"))); // flat map 내부 처리는 String으로 이루어지고 바로 나온다.
